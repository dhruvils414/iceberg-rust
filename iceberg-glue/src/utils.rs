...
use aws_sdk_glue::types::{DatabaseInput, StorageDescriptor, TableInput};
use iceberg_rust::{
...
/// Create `DatabaseInput` from `Namespace` and properties
...
/// Checks if provided `Namespace` is valid
pub(crate) fn validate_namespace(namespace: &Namespace) -> Result<String> {
...
mod tests {
    ...
    fn test_convert_to_database() -> Result<()> {
        let namespace = Namespace::new("my_database".to_string());
